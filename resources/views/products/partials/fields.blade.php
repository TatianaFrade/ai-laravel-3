...
<div class="w-full sm:w-96">
    @if ($readonly || (!$canEditAll && !$canEditStockOnly))
        <flux:input name="name" label="{{ __('Name') }}" value="{{ old('name', $product->name ?? '') }}" readonly :placeholder="__('Required')" />
        <input type="hidden" name="name" value="{{ old('name', $product->name ?? '') }}">
    @else
        <flux:input name="name" label="{{ __('Name') }}" value="{{ old('name', $product->name ?? '') }}" :placeholder="__('Required')" />
    @endif
</div>
