<div class="max-w-2xl md:max-w-3xl mx-auto p-8 bg-white text-gray-900 dark:bg-gray-900 dark:text-gray-100 shadow-xl rounded-md">
    <div class="flex flex-col items-center gap-6 md:gap-8">
        <!-- Product Image -->
        <div class="w-96 h-96 bg-gray-200 dark:bg-gray-800 flex justify-center items-center rounded-md">
            <img src="{{ $product->image_url }}" alt="{{ $product->name }}" class="w-88 h-88 object-cover rounded-md">
        </div>

        <!-- Product Details -->
        <div class="w-full p-6 bg-gray-100 dark:bg-gray-800 rounded-md text-center">
            <h1 class="text-4xl font-bold text-black dark:text-white">{{ $product->name }}</h1>
            <p class="text-gray-600 dark:text-gray-400 mt-4 text-base">{{ $product->description }}</p>
            ...
            <!-- Price and Discount -->
            <div class="mt-6 bg-gray-200 dark:bg-gray-700 p-4 rounded-md">
                @if ($product->discount > 0 && $product->stock > $product->discount_min_qty)
                    ...
                    <p class="text-xl font-bold text-gray-800 dark:text-gray-300">
