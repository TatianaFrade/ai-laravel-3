...
<flux:sidebar sticky stashable class="border-e border-zinc-200 bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-900 flex flex-col h-full">
    <div class="flex-none">
        ...
        @auth
            <flux:dropdown position="bottom" align="start">
                <flux:profile :name="auth()->user()?->firstLastName() ?? auth()->user()->name"
                    ...
                    :avatar="auth()->user()?->imageUrl" icon-trailing="chevrons-up-down" />
                            ...
                            <div class="flex items-center gap-2 px-1 py-1.5 text-start text-sm">
                                <span class="relative flex h-8 w-8 shrink-0 overflow-hidden rounded-lg">
                                    @if(auth()->user()?->imageUrl)
                                        <img src="{{ auth()->user()->imageUrl }}" alt="User Photo" class="rounded-full w-8 h-8 object-cover" />
                                    @else
