...
class OrderController extends Controller
{
    ...
    public function update(OrderFormRequest $request, Order $order)
    {
        ...
        if ($statusChangedToCompleted) {
            ...
            if (!$order->user) {
                \Log::error('Usuário não encontrado para order id: ' . $order->id . ', member_id: ' . $order->member_id);
                return back()->withErrors(['user' => 'Usuário associado não encontrado para esta encomenda.']);
            }
